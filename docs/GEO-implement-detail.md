...
- 从 robots.txt 内容解析所有 `Sitemap:` URL，对每个 sitemap index URL 发起 GET 请求，通过标准是每个sitemap URL均可正常访问且内容非空
- 对每个 URL 发起 HEAD 请求，HTTP 状态码为 **200**
- 从全部 sitemap 条目中**随机抽样 10 个**，检查每个抽样条目是否定义了 `priority` 属性
...
- **openGauss:** 文章类页面（blogs/news/user-practice/events）不归档于 `.geo` 目录，沿用源md文档自身就有的title、description，并由脚本从frontmatter中提取字段信息生成JSON-LD
